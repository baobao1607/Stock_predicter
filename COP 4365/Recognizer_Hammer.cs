...
{
    /// <summary>
    /// this class represents for the hammer pattern recognizer. It is inherited from the recognizer abstract class
    /// </summary>
    internal class Recognizer_Hammer : Recognizer
    {
        /// <summary>
        /// This method recognize if the candlestick has a patttern hammer or not. It takes in a list of smart candlestick 
        ...
        /// <returns>A boolean value whether it forms a hammer pattern or not</returns>
        override public bool Recognize(List<SmartCandlestick> lscs, int index)
        {
            ...
            if (!there)
            {
                // check whether it is a hammer candlestick or not (boolean type)
                result = (scs.upperTail < 0.03m * scs.Range) && (scs.bodyRange >= 0.2m * scs.Range) && (scs.bodyRange <= 0.3m * scs.Range);
                // add in the dictionary with the pattern name as key and value is the boolean value
                scs.dictionary_patterns.Add("Hammer", result);
            }
            //return the boolean value whether it forms a hammer pattern or not
            return result;
        }

        /// <summary>
        /// This method is the default constructor for the hammer recognizer class
        /// It passes in string hammer into the pattern name and 1 into the number of smartcandlestick objects used.
        /// </summary
