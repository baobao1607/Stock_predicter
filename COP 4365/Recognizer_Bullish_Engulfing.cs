...
{
    /// <summary>
    /// this class represents for the Bullish_Engulfing pattern recognizer. It is inherited from the recognizer abstract class
    /// </summary>
    internal class Recognizer_Bullish_Engulfing : Recognizer
    {
        /// <summary>
        /// This method is the default constructor for the Bullish_Engulfing recognizer class
        /// It passes in string Bullish_Engulfing into the pattern name and 2 into the number of smartcandlestick objects used.
        /// </summary
        public Recognizer_Bullish_Engulfing() : base("Bullish_Engulfing", 2) { }
        /// <summary>
        /// This method recognize if the candlestick has a patttern Bullish_Engulfing or not. It takes in a list of smart candlestick 
        ...
        /// <returns>A boolean value whether it forms a Bullish_Engulfing pattern or not</returns>
        override public bool Recognize(List<SmartCandlestick> lscs, int index)
        {
            ...
            if (!there)
            {
                ...
                if (index - 1 < 0)
                {
                    ...
                    //return the boolean value whether it forms a Bullish_Engulfing pattern or not
                    return false;
                }
                // if it is not, check if it forms a Bullish_Engulfing or not
                else
                {
                    //assign the previous smartcandlestick object to variable scs_previous
                    SmartCandlestick scs_previous = lscs[index - 1];
                    // check whether it is a Bullish_Engulfing candlestick or not (boolean type)
                    r = (scs.close > scs.open) && (scs_previous.close < scs_previous.open) && (scs.close > scs_previous.open) && (scs.open < scs_previous.close);
