...
{
    /// <summary>
    /// this class represents for the Bearish pattern recognizer. It is inherited from the recognizer abstract class
    /// </summary>
    internal class Recognizer_Bearish : Recognizer
    {
        /// <summary>
        /// This method recognize if the candlestick has a patttern Bearish or not. It takes in a list of smart candlestick 
        ...
        /// <returns>A boolean value whether it forms a Bearish pattern or not</returns>
        override public bool Recognize(List<SmartCandlestick> lscs, int index)
        {
            ...
            if (!there)
            {
                // check whether it is a Bearish candlestick or not (boolean type)
                result = scs.close < scs.open;
                // add in the dictionary with the pattern name as key and value is the boolean value
                scs.dictionary_patterns[patternName] = result;
            }
            //return the boolean value whether it forms a Bearish pattern or not
            return result;
        }
        /// <summary>
        /// This method is the default constructor for the Bearish recognizer class
        /// It passes in string Bearish into the pattern name and 1 into the number of smartcandlestick objects used.
        /// </summary
