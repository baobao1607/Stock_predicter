...
{
    /// <summary>
    /// this class represents for the Bearish_Harami pattern recognizer. It is inherited from the recognizer abstract class
    /// </summary>
    internal class Recognizer_Bearish_Harami : Recognizer
    {
        /// <summary>
        /// This method is the default constructor for the Bearish_Harami recognizer class
        /// It passes in string Bearish_Harami into the pattern name and 2 into the number of smartcandlestick objects used.
        /// </summary
        public Recognizer_Bearish_Harami() : base("Bearish_Harami", 2) { }
        /// <summary>
        /// This method recognize if the candlestick has a patttern Bearish_Harami or not. It takes in a list of smart candlestick 
        ...
        /// <returns>A boolean value whether it forms a Bearish_Harami pattern or not</returns>
        override public bool Recognize(List<SmartCandlestick> lscs, int index)
        {
            ...
            if (!there)
            {
                ...
                if (index - 1 < 0)
                {
                    ...
                    //return the boolean value whether it forms a Bearish_Harami pattern or not
                    return false;
                }
                // if it is not, check if it forms Bearish_Harami or not
                else
                {
                    //assign the previous smartcandlestick object to variable scs_previous
                    SmartCandlestick scs_previous = lscs[index - 1];
                    // check whether it is a Bearish_Harami candlestick or not (boolean type)
