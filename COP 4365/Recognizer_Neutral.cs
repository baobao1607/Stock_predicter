...
{
    /// <summary>
    /// this class represents for the neutral pattern recognizer. It is inherited from the recognizer abstract class
    /// </summary>
   
    internal class Recognizer_Neutral : Recognizer
    {
        /// <summary>
        /// This method recognize if the candlestick has a patttern neutral or not. It takes in a list of smart candlestick 
        ...
        /// <returns>A boolean value whether it forms a neutral pattern or not</returns>
        override public bool Recognize(List<SmartCandlestick> lscs, int index)
        {
            ...
            if (!there)
            {
                // check whether it is a neutral candlestick or not (boolean type)
                result = Math.Abs(scs.close - scs.open) < 0.5m;
                // add in the dictionary with the pattern name as key and value is the boolean value
                scs.dictionary_patterns.Add("Neutral", result);
            }
            //return the boolean value whether it forms a neutral pattern or not
            return result;
        }
        /// <summary>
        /// This method is the default constructor for the neutral recognizer class
        /// It passes in string neutral into the pattern name and 1 into the number of smartcandlestick objects used.
        /// </summary>
